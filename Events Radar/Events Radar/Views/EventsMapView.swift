            ...
            ZStack {
                ...
                VStack {
                    Text(locationLabel)
                        ...
                        .foregroundColor(Color(hue: 0.553, saturation: 0.784, brightness: 0.644))
                        .background(.white)
                        .padding(.top, -3)
                        .opacity(0.9)
                    
                    Text("Lat: " + String(format:"%.5f%@", locationLat, hemiNS) + ", " + "Lon: " + String(format:"%.5f%@", locationLon, hemiEW))
...
enum MapDetails {
    static let startingLocation = CLLocationCoordinate2D(latitude: 19, longitude: -80)
    static let defaultSpan = MKCoordinateSpan(latitudeDelta: 100, longitudeDelta: 100)
    static let currentSpan = MKCoordinateSpan(latitudeDelta: 0.01, longitudeDelta: 0.01)
}
        ...
        if CLLocationManager.locationServicesEnabled() {
            ...
            addressManager.reverseGeocodeLocation(loc, completionHandler:
                ...
                if pm?.count ?? 0 > 0 {
                    ...
                    print("Coordinates Rounded: ", String(format:"%.5f", locationLat) + ", " + String(format:"%.5f", locationLon))
                    print(hemiNS, hemiEW)
                }
            })
        } else {
